...
//
// Created by chaochao on 2024/4/23.
//

#ifndef LUCIDADASTORAGE_H
#define LUCIDADASTORAGE_H
...
namespace lucidstorage {
    ...
    public:
        ...
        lucidmemory( unsigned long int numkey,double Threshold, index_type *lucidtree , lru_type *LRU, int bitnum){
            totalnum = 0;
            threshold = Threshold;
//            evicttable.reserve(numkey);
        ...
        pair<key_type, key_type *> get_key(key_type key, stat_type query_stats, disk_type* disklucid){
...
//                    findpiece->intrachain.moveTohead(finddata);// update intrachain
...
//                    pair<pageid_type, pageOff_type>  reduced_writeevict (0,0);
                    ...
                    // 在lucid+ flattened structures 中， 为进一步降低 memory usage，改成了flattened structure 的结构
//                    gettimeofday(&xt1, NULL);

                    auto int_reduced_evict = short_reduced_evict/2.0;
                    unsigned long index_pageid = floor(int_reduced_evict/disklucid->reduce_factor)*disklucid->reduce_factor;
...
//                    gettimeofday(&xt2, NULL);
//                    xtimeuse = (xt2.tv_sec - xt1.tv_sec) + (double) (xt2.tv_usec - xt1.tv_usec) / 1000000.0;
//                    query_stats->flattenedtime += xtimeuse;
...
//                    gettimeofday(&prdt1, NULL);
                    if (pageid == inpage->pageid) {
                        ...
                        res = disklucid->odirectreadfromdisk(pageid,offset,this,index_pageid);    // if readfromdisk indicating doesn't use o_direct;
                    }
...
//                    pair<pageid_type, pageOff_type>  reduced_writeevict (0,0);
//                            auto writeevict = (pair<pageid_type, pageOff_type> *) index_res.findleaf->slotdata[index_res.slot];
...
//                    gettimeofday(&xt1, NULL);
                    ...
                    // 根据reduced_evict 算出（读出） page id，算出读出offset
                    if (fraction){
                        pageoff = int_high_low&0xFFFF;  // 低16位
                    }
                    else
                        pageoff =  int_high_low>>16 ;   // 高16位

                    unsigned long index_pageid = floor(int_reduced_evict/disklucid->reduce_factor)*disklucid->reduce_factor;  // 64k pagesize 时
...
//                    gettimeofday(&prdt1, NULL);
                    if (pageid == inpage->pageid) {
                        ...
                        res = disklucid->odirectreadfromdisk(
...
//                    query_stats->lrutime += pltimeuse;
        ...
        template< typename stat_type>
        pair<key_type, key_type *> update_key(key_type key, key_type* new_payload, stat_type query_stats, disk_type disklucid){
            struct timeval tqt1, tqt2, xt1, xt2, plt1, plt2, prdt1, prdt2;
            double pltimeuse, prdtimeuse, pwdtimeuse, timeuse, xtimeuse;  // 读磁盘的时间，写磁盘的时间
            pair<key_type, key_type *> res;
            gettimeofday(&xt1, NULL);
...
//                    gettimeofday(&plt1, NULL);
                    ...
                    findpiece->slotdata[index_res.slot] = new_payload;
                    findpiece->slotflag[index_res.slot] = true;
...
//                    gettimeofday(&prdt1, NULL);
                    if (reduced_writeevict.first == inpage->pageid) {
                        ...
                        res = disklucid.odirectreadfromdisk(pageid, pageoff,this, index_pageid);    // if readfromdisk indicating doesn't use o_direct;
                    }
//                    gettimeofday(&prdt2, NULL);
//                    prdtimeuse = (prdt2.tv_sec - prdt1.tv_sec) +
//                                 (double) (prdt2.tv_usec - prdt1.tv_usec) / 1000000.0;
...
//                    query_stats->lrutime += pltimeuse;
        ...
        template< typename stat_type>
        pair<key_type, key_type *> scan_key(key_type key, int scan_num, stat_type query_stats, disk_type disklucid){

        }

        template< typename stat_type>
        typename index_type::htaprange_res_find get_range(const key_type firstkey, const key_type lastkey, stat_type query_stats, disk_type disklucid){
//            struct timeval tqt1, tqt2, xt1, xt2, plt1, plt2, prdt1, prdt2;
//            double pltimeuse, prdtimeuse, pwdtimeuse, timeuse, xtimeuse;  // 读磁盘的时间，写磁盘的时间
//            gettimeofday(&xt1, NULL);
...
//            gettimeofday(&xt2, NULL);
        ...
        template< typename stat_type>
        typename index_type::htaprange_res_find get_scan(const key_type firstkey, int scan_num, stat_type query_stats, disk_type disklucid){
//            struct timeval tqt1, tqt2, xt1, xt2, plt1, plt2, prdt1, prdt2;
//            double pltimeuse, prdtimeuse, pwdtimeuse, timeuse, xtimeuse;  // 读磁盘的时间，写磁盘的时间
//            gettimeofday(&xt1, NULL);
...
//                diskpage->odirectenterpage(inpage->inmemdata);// write to disk
        ...
        unsigned long lucidtransfer(unsigned int transleaves,disk_type *diskpage){   //perform the transfer procedure,
            //
...
//                        delete transdata;
        ...
        unsigned long lucidtransfer(double totalusemem,disk_type *diskpage){   //perform the transfer procedure,
            //
            timeval initw1,initw2;
            unsigned long initwtime = 0;
...
//            cout<<endl;
    ...
    public:
        ...
        luciddisk( const char* diskfile,int sizepage,int numrecord,int sizerecord, int merge_setting) {
            pagefile = diskfile;
            pagesize = sizepage;
            recordnum = numrecord;
...
//            free(buf);
        ...
        pair<key_type,key_type*> odirectreadfromdisk(pageid_type pageid,pageOff_type offset, memory_type memorylucid,
...
//            bool former_flag = memorylucid->check_exists(recordnum,index_pageid);
...
//                cout << "i need You, my Lord, the flag is wrong" << endl;
            ...
            unset, that is let the key's bit set to 0, means that the key in page is out of date
             modify bitmap
            */
            memorylucid->unset_bit(recordnum, index_pageid);
//            bool after_flag = memorylucid->check_exists(recordnum,index_pageid);   // int pos, unsigned long evictpos, unsigned long index_pageid
//            if (after_flag == true)
//                cout << "i need You, my Lord! please come! unset bit wrong " << endl;
            // check merge
            if (memorylucid->check_merge(index_pageid) == true){
                // record the index_pageid, that the position in the reduced_evictedtable, which can be reused.
                memorylucid->reused_pageids.emplace_back(pageid );
                memorylucid->num_page_merge += 1;
                memorylucid->reused_index_pageid.emplace_back(index_pageid);
                memorylucid->page_merge(buf,this->recordnum, index_pageid);
                memorylucid->index->exkeynum -= memorylucid->reduced_evicttable[index_pageid+1];
                memorylucid->index->inkeynum += memorylucid->reduced_evicttable[index_pageid+1];
                memorylucid->reduced_evicttable[index_pageid+1] = 0;
