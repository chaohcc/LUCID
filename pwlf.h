...
namespace lucidinsert::internal{
//    typedef lucidinsert::Leafpiece sort_piece_type;
...
//            }
    ...
    template<typename key_type,typename lucidtype>
    std::pair<size_t,std::vector<key_type> > append_segmentation(size_t error,std::vector<key_type> keys,
                                                                 lucidtype *lucidada,unsigned int k){
        size_t c = 0;
        std::vector<key_type> startkeys;
         // innerpiece;
        for (size_t i = 0; i < keys.size(); ++i) {
            pair<key_type,int> p(keys[i],lucidada->innerlevels[k]->nextpos++) ;   // i 为 pos
            ++(lucidada->innerlevels[k]->pos);
            if (!lucidada->innerlevels[k]->opt->add_point(p.first, p.second)) {  // 如果inner level  不满足error 了，那么再创建一个innerpiece

//                lucidada->i_innerpiece = innerpiece;
                auto a = lucidada->innerlevels[k]->opt->get_segment();
                lucidada->innerlevels[k]->i_innerpiece->update(a);
                if (lucidada->innerlevels[k]->pos > 2)
                    lucidada->innerlevels[k]->innerpieces.pop_back();
                lucidada->innerlevels[k]->innerpieces.emplace_back(lucidada->innerlevels[k]->i_innerpiece);
                // 当前 innerpiece 不再满足，需要创建new inner piece 并判断该 innerlevel 的上一层level 是否需要更新
                typename lucidtype:: Innerpiece* innerpiece = new typename lucidtype:: Innerpiece();
                lucidada->innerlevels[k]->i_innerpiece = innerpiece;
                // 首先在该层创建一个 new innerpiece， 更新该innerpiece，再递归向向上

                lucidada->innerlevels[k]->pos = 0;
                lucidada->innerlevels[k]->nextpos -= 2;
                delete lucidada->innerlevels[k]->opt;
//                lucidada->innerlevels[k]->opt.pop_back();
                insertPWLF<key_type, int> *inneropt = new insertPWLF<key_type, int>(error);
                lucidada->innerlevels[k]->opt=inneropt;
                if (k==0){
//                    auto aaaaa = lucidada->leaflevel.leafpieces.size()-2;
                    startkeys.emplace_back(lucidada->leaflevel.leafpieces[lucidada->leaflevel.leafpieces.size()-1]->startkey);
                }

                else{
//                    auto aaaaa = lucidada->innerlevels[k-1]->innerpieces.size()-2;
                    startkeys.emplace_back(lucidada->innerlevels[k-1]->innerpieces[lucidada->innerlevels[k-1]->innerpieces.size()-2]->startkey);
                }
                startkeys.emplace_back(p.first);
                auto rr = append_segmentation(error,startkeys,lucidada,k);

                ++c;
                if (lucidada->innerlevels.back()->innerpieces.size() > 1)
                {
                    startkeys.clear();
                    ...
                    insertPWLF<key_type, int> *inneropt = new insertPWLF<key_type, int>(error);
//
                    typename lucidtype::Innerlevel *innerlevel = new typename lucidtype::Innerlevel;
                    lucidada->innerlevels.emplace_back(innerlevel);
                    lucidada->innerlevels.back()->opt = inneropt ;
                    auto rr = append_segmentation(error,startkeys,lucidada,k+1);
//                    cout<< "Jesus, i need You !"  << endl;
                    startkeys.emplace_back(a.first);
                    return std::pair<size_t,std::vector<key_type> >(++c,startkeys) ;
                }
                else if (lucidada->innerlevels.size() > 1 && (k != lucidada->innerlevels.size()-1) )   // 如上为 由于创建了new innner piece，导致了new innerlevel，如下为，虽然创建了new innerpiece，但只需要更新上层的innner level
                {
...
//                    cout << "thank You, my Lord! i need You!"<<endl;
                ...
                a = lucidada->innerlevels[k]->opt->get_segment();
                lucidada->innerlevels[k]->i_innerpiece->update(a);
                if (lucidada->innerlevels[k]->pos > 2)
                    lucidada->innerlevels[k]->innerpieces.pop_back();
                lucidada->innerlevels[k]->innerpieces.emplace_back(lucidada->innerlevels[k]->i_innerpiece);
...
//
    template<typename key_type, typename leaf_type,typename lucidtype,typename lucidadalrutype>
    std::pair<size_t,std::vector<key_type> > append_segmentation(size_t error,std::vector<key_type> keys,key_type* payload,
                                                                 lucidtype *lucidada, unsigned long int &inkeynum,leaf_type* m_tailleaf,lucidadalrutype *interchain){
...
//        }
//        int valuesize = lucidada->valuesize;
        inkeynum +=1;
        ...
        if (m_tailleaf == NULL)  // 初始化
        {
            ...
            lucidada->m_tailleaf = cur_leaf;
            lucidada->leaflevel.leafpieces.emplace_back( lucidada->m_tailleaf);
//            m_tailleaf = cur_leaf;
            insertPWLF<key_type, int> *leafopt = new insertPWLF<key_type, int> (error);
            lucidada->leaflevel.opt = leafopt ;

        }
//        auto xx = lucidada->leaflevel.second[0]->buffqueue.size();
        lucidada->leaflevel.opt->append_point(p.first, p.second);
//        key_type* value = new key_type[valuesize];
        lucidada->m_tailleaf->slotdata.emplace_back( lucidada->m_tailleaf->intrachain.put(p.second,payload));
        lucidada->m_tailleaf->slotkey.emplace_back(p.first);
        lucidada->m_tailleaf->slotflag.emplace_back(true);
...
//            }
            ...
            if (inkeynum != start && next_p.first == p.first)
                continue;
            p = next_p;
            if (lucidada ->leaflevel.opt->append_point(p.first, p.second)) {
                lucidada->m_tailleaf->slotkey.emplace_back(p.first);
//                key_type* value = new key_type[valuesize];
                lucidada->m_tailleaf->slotdata.emplace_back( lucidada->m_tailleaf->intrachain.put(p.second,payload));
                lucidada->m_tailleaf->slotflag.emplace_back( true);
            ...
            {
                start = inkeynum;
                auto a = lucidada ->leaflevel.opt->get_segment(keys[--inkeynum]); // 将生成的new leaf piece插入到leaflevel 中
                lucidada->m_tailleaf->update(a);
//                lucidada ->leaflevel.leafpieces.emplace_back( lucidada->m_tailleaf);
                interchain->put(lucidada->m_tailleaf->startkey,lucidada->m_tailleaf);
//                if (lucidada->m_tailleaf->startkey == 12367821099355)
//                    COUT_THIS('thank You, my lovely Lord');
                auto buffersize = ceil(lucidada->m_tailleaf->slotkey.size()*lucidada->buffer_ratio)+1;   //0.429, 0.25,0.667
                lucidada->m_tailleaf->buffer_piece = new typename lucidtype:: sortPiece(buffersize);

                if (buffersize < lucidada->minbsize)
                    lucidada->minbsize = buffersize;
                else if (buffersize > lucidada->maxbsize)
                    lucidada->maxbsize = buffersize;
                lucidada->m_tailleaf->buffer_piece->slope = buffersize;
                lucidada->m_tailleaf->buffer_piece->slotkey.reserve(buffersize);
                lucidada->m_tailleaf->buffer_piece->slotdata.reserve(buffersize);
                lucidada->m_tailleaf->buffer_piece->slotflag.reserve(buffersize);
                // 这里是初始化 parent piece 的first key 和 last key
                if (lucidada->innerlevels.size() == 0){
                    startkeys.emplace_back( lucidada->m_tailleaf->startkey);
                    startkeys.emplace_back( p.first);
//                    typename lucidtype:: Innerpiece* innerpiece = new typename lucidtype:: Innerpiece() ;// 创建parent piece
//                    lucidada->i_innerpiece = innerpiece;
                    insertPWLF<key_type, int> *inneropt = new insertPWLF<key_type, int>(error);
                    typename lucidtype::Innerlevel *innerlevel = new typename lucidtype::Innerlevel;
                    lucidada->innerlevels.emplace_back(innerlevel);
                    lucidada->innerlevels[0]->opt = inneropt ;
...
//                    cout << "my lovely Lord, i trust in You!" << endl;
                ...
                auto rr = append_segmentation(error,startkeys,lucidada,0);
//                startkeys.clear();
                vector<key_type>().swap(startkeys);
//                cout << "Jesus, i need You!!"<< endl;
                pos = 0;

                lucidada->m_tailleaf = new leaf_type;
                lucidada->m_tailleaf->slotkey.reserve(8192*4);
                lucidada->m_tailleaf->slotdata.reserve(8192*4);
                lucidada->m_tailleaf->slotflag.reserve(8192*4);
                lucidada ->leaflevel.leafpieces.emplace_back( lucidada->m_tailleaf);
                ++c;
            }
        }
        auto a = lucidada ->leaflevel.opt->get_segment( keys.back());
        ...
        lucidada->m_tailleaf->update(a);
        interchain->put(lucidada->m_tailleaf->startkey,lucidada->m_tailleaf);
        auto buffersize = (lucidada->maxbsize+lucidada->maxbsize)/2;
        lucidada->m_tailleaf->buffer_piece = new typename lucidtype:: sortPiece(buffersize);
        lucidada->m_tailleaf->buffer_piece->slope = buffersize;
        lucidada->m_tailleaf->buffer_piece->slotkey.reserve(buffersize);
        lucidada->m_tailleaf->buffer_piece->slotdata.reserve(buffersize);
    ...
    template<typename key_type, typename lucidtype,typename lucidadalrutype>
    typename lucidtype::sortPiece* make_segmentation(lucidtype *lucidindex,size_t error,std::vector<key_type> retrainkeys,
                                                    std::vector<bool> retrainflags, std::vector<void*> retraindata,
                                                    lucidadalrutype *interchain,int leafslot){

//        size_t c = 0;
        size_t start = 0;
        unsigned int pos = 0;
        vector<typename lucidtype::Leafpiece*> newleafs;
        typename lucidtype::Leafpiece* curleaf = new typename lucidtype::Leafpiece;
        insertPWLF<key_type, int> *leafopt = new insertPWLF<key_type, int> (error);
...
//            }
            ...
            if (leafopt->append_point(p.first, p.second)) {
                ...
                if (retrainflags[i] == true){
                    curleaf->slotflag.emplace_back(true);
                    ...
                    curleaf->slotflag.emplace_back(false);
                }
            }
            else
            {
...
//                    cout << "i need you, my Lord, the next leaf is too small, so the remaining key should be inserted into the buffer piece"<< endl;
                    auto buffersize = ceil(newleafs[0]->slotkey.size()*lucidindex->buffer_ratio)+4;
                    typename lucidtype::sortPiece* bufferpiece = new typename lucidtype:: sortPiece(buffersize);
...
//                            cout << "i need You, my lovely Lord, rebuild 17386164347307" << endl;
                        auto predicted_pos = (*newleafs[0])(key)*lucidindex->buffer_ratio;
                        if (retrainflags[i] == true){
                            auto finddata = (adalru::Node<lruOff_type, key_type* >*) retraindata[i];
...
//                    }
                    ...
                    auto buffersize = ceil(newleafs[0]->slotkey.size()*lucidindex->buffer_ratio)+1;
                    typename lucidtype::sortPiece* bufferpiece = new typename lucidtype:: sortPiece(buffersize);
                    curleaf->buffer_piece = bufferpiece;
                    pos = 0;
                    delete leafopt;
                    ...
                    curleaf = new typename lucidtype::Leafpiece;
//                    ++c;
                }
            }
        }
...
//                cout << "Jesus, i need You, curleaf->intrachain.size == 0" << endl;
            newleafs.emplace_back(curleaf);
            auto buffersize = ceil(newleafs[0]->slotkey.size()*lucidindex->buffer_ratio)+1;
            curleaf->buffer_piece = new typename lucidtype:: sortPiece(buffersize);
...
//            curleaf->buffer_piece->slotflag.reserve(buffersize);
...
//            ++c;
        ...
        lucidindex->leaflevel.leafpieces[leafslot] = newleafs[0];
//        lucidtype::delete_leaf(leaf); // 释放original leaf
//        auto original1 = lucidindex->leaflevel.leafpieces[leafslot];
...
//        }
        ...
        if (c>1){
            typename lucidtype::Leafpiece* curleaf = newleafs[0];
...
//            cout << "Jeses, i need You! c = 0"<< endl;
        ...
        //
        return  (typename lucidtype:: sortPiece*)curleaf->buffer_piece;
    ...
    typename lucidtype::sortPiece* make_segmentation(lucidtype *lucidindex,size_t error,std::vector<key_type> retrainkeys,
                                                    std::vector<bool> retrainflags, std::vector<void*> retraindata,
                                                    lucidadalrutype *interchain,vector<typename lucidtype::Leafpiece*>& linkpieces, int li){
...
//            cout << "Jeses, i need You! c = 0"<< endl;
    ...
    template< typename lucidtype ,typename key_type,typename leaf_level_type,typename innerlevel_type, typename lru_type>
    std::pair<unsigned int ,vector<key_type>>  make_segmentation(lucidtype* lucidindex,unsigned short error,leaf_level_type *leaflevel, innerlevel_type *innerlevel, key_type firstkey, lru_type interchain) {
        ...
        for (unsigned int i = 0; i < leaflevel->leafpieces.size(); ++i) {
            ...
            if (curleaf->buffer_piece->buffer_piece) // 如果有link piece
            {
                ...
                typename lucidtype::sortPiece* bufferpiece = (typename lucidtype::sortPiece*)curleaf->buffer_piece;
...
//                        auto buffersize = ceil(linkpieces[pi]->slotkey.size()*lucidindex->buffer_ratio);
//                        linkpieces[pi]->buffer_piece = new typename lucidtype::sortPiece(buffersize);
////                        if (buffersize < lucidindex->minbsize)
////                            lucidindex->minbsize = buffersize;
////                        else if (buffersize > lucidindex->maxbsize)
////                            lucidindex->maxbsize = buffersize;
//                    }
//                }

                // 每一个 leaf 的 buffer 都 new 一个 sort piece
                for (auto piece: linkpieces){
                    auto buffersize = ceil(piece->slotkey.size()*lucidindex->buffer_ratio)+4;
                    piece->buffer_piece = new typename lucidtype::sortPiece(buffersize);
...
//                            COUT_THIS("thank You, my Lord");
                        for (; li<linkpieces.size();li++){
                            if (bufferkey <= linkpieces[li]->startkey)
                                break;}
                        li--;
                        typename lucidtype::sortPiece* buffer = (typename lucidtype::sortPiece*) (linkpieces[li]->buffer_piece);
//                        buffer->data_capacity_ +=1;
                        auto predicted_pos = (*linkpieces[li])(bufferkey)*lucidindex->buffer_ratio;
                        ...
                        if (buffer->num_keys_ == buffer->data_capacity_){
                            ...
                            bufferpiece = internal::make_segmentation( lucidindex,error,retrain_keys,retrain_flags,retrain_datas,interchain,linkpieces,li) ;
//                this->newmodels += (size_buffer.first- link_num);
                            // 执行插入

                            predicted_pos = (*linkpieces[li])(bufferkey) * lucidindex->buffer_ratio;  // normalize to buffer size;
...
//                delete bufferpiece;
                ...
                for (auto leaf:linkpieces){
                    ...
                    typename lucidtype::sortPiece* buffer = (typename lucidtype::sortPiece*) (leaf->buffer_piece);
...
//                lucidada->innerlevels[k]->opt.pop_back();
        ...
        innerlevel->i_innerpiece->update(a);
        innerlevel->innerpieces.emplace_back(innerlevel->i_innerpiece);
        vector<typename lucidtype::Leafpiece*>().swap(leaflevel->leafpieces);
    ...
    template< typename lucidtype ,typename key_type,typename leaf_level_type,typename innerlevel_type>
    std::pair<unsigned int ,vector<key_type>>  make_segmentation(lucidtype* lucidindex,unsigned short error,leaf_level_type *leaflevel, innerlevel_type *innerlevel, key_type firstkey) {
...
//                lucidada->innerlevels[k]->opt.pop_back();
    ...
    template<typename lucidtype, typename key_type,typename innerlevel_type>
    std::pair<unsigned int,vector<key_type> > make_segmentation(lucidtype *lucidindex,size_t error,std::vector<key_type> keys,innerlevel_type *innerlevel){
