...
#define PGM_ADD_EPS(x, epsilon, size) ((x) + (epsilon) + 2 >= (size) ? (size) : (x) + (epsilon) + 2)
    ...
    struct access_stats {
        ...
        void print_stats(){
            ...
            ofstream savefile2;
            string performance_file2 = "/home/wamdm/chaohong/clionDir/updatelucid/result/lucid_performance.txt";
...
// True if a == b
    ...
    public:
        ...
        //    typedef lucidinsert::lucidinsert< key_type, key_type* > lucidadatype;
        typedef lucidinsert::LUCIDinsert<key_type, key_type*, AlexCompare, std::allocator<std::pair<key_type, key_type*>>, true> lucidadatype;
...
//            int slotlo = PGM_SUB_EPS2(learnedPos, Error + 1,(this->slotkey.size()-1)) ;  //,(--itlevel)->size()
                ...
                retrainflag.insert(retrainflag.end(),this->slotflag.begin()+slot,this->slotflag.begin()+resslot);
...
// 依次比较 buffer 和 this->slotkey 中的元素，哪个小就插入到slotkey 中
                    if (slot < this->slotkey.size()) {
                        if (key < this->slotkey[slot]) {
                            ...
                            retrainflag.emplace_back(this->slotflag[slot++]);
                        }
                    } else {
...
//            auto nowkey = this->slotkey[slot];
                ...
                retrainflag.insert(retrainflag.end(),this->slotflag.begin()+slot,this->slotflag.end());
...
//            要将bufferpiece 从interchain 也就是global LRU中删除
                ...
                vector<bool>().swap(this->slotflag);
...
slotflag.emplace_back(bufferpiece->pointer_slots_[i].first);
} else{
i --;
slotkey.emplace_back(this->slotkey[slot]);
slotdata.emplace_back(this->slotdata[slot]);
...
//        lucidinsert() = default;
        ...
        LUCIDinsert(unsigned int n ,int err1,int err2)
        {
...
//                level_it->innerpieces.clear();
        ...
        /*
         * the method to construct lucid, that build lucid
         */
...
//                        Innerpiece innerpiece;// 创建parent piece
                        internal::insertPWLF<key_type, int> *inneropt = new internal::insertPWLF<key_type, int>(error);
//                    std::pair< std::vector<typename lucidtype:: Innerpiece>, std::vector<internal::insertPWLF<key_type, int>*> > *innerlevel =
//                            new std::pair< std::vector<typename lucidtype:: Innerpiece>, std::vector<internal::insertPWLF<key_type, int>*> >;
                        Innerlevel *innerlevel = new Innerlevel;
                        innerlevels.emplace_back(innerlevel);
...
//                cout<< "i need You, my lovely Lord!" << endl;
        ...
        inline void update_append(std::vector<key_type> keys, key_type* payload,unsigned int error,unsigned int error_recursize,lru_type &interchain ){
            ...
            //lucidtype *lucidada, unsigned int &inkeynum,leaf_type* m_taillea
...
//            { }
        ...
        /// lucid recursive find the key has much information which is needs to be return.
        struct result_find
        {
            /// find result flags
...
//
//                    auto model_res = buffer->find_key(key, resslot);
//                    if (model_res >0 && model_res == buffer->data_capacity_)
//                        model_res -= 1;
//                    index_res.slots.push_back(model_res);
//                    return index_res;
//                }
...
//                Leafpiece* a = *curleaf;
//
//                result_find index_res= result_find(false,a->slotflag[0],a,0);   // 默认 数据是不重复的，
...
//                        COUT_THIS("please come, my lovely Lord, key != newlrukey")
        ...
        inline void verify_lucid(lucid_stats *fstats){
        ...
        const map<string, double> show_verify()
        {
            ...
            double buffernodesize = 12;  // without start key, has: capacity(4), num in buf(4), bitmapsize（4）, (slotflag, slot key, slot data: 预分配空间)
...
//    typedef lucidinsert::lucidinsert< key_type, key_type* > lucidadatype;
    ...
    typedef pair<lucidadatype::Leafpiece*,unsigned short int>  lucidadalrupair;  //lru 中value 的type ，是一个pair，first is 所属的leaf，second 是slot in leaf
    typedef  adalru::hashLRU <key_type,lucidadatype::Leafpiece* , adalru::Node<key_type ,lucidadatype::Leafpiece*>* > lucidadalrutype;
    typedef adalru::localLRU <unsigned short,key_type* > locallrutype;
    typedef lucidstorage::luciddisk<key_type> lucidadadisk;
    typedef lucidstorage::lucidmemory<key_type,key_type*,lucidadatype, lucidadalrutype,lucidadadisk> lucidadamemory;
    typedef lucidadamemory::memoryusage memoryusage;
    //    adalru::hashLRU <key_type,Leafpiece* , adalru::Node<key_type ,Leafpiece*>* >


    double runtimeevictkeytopage(lucidadamemory *lucidmem,double totalusemem, lucidadatype *lucidindex,lucidadadisk *diskpage , access_stats *r_stats){
        struct timeval wdt1,wdt2,lt1,lt2;
        double wdtimeuse =0.0;  // 写磁盘的时间
        double ltimeuse = 0.0;

        if (lucidmem->evictPoss.size() != 0){
            gettimeofday(&wdt1, NULL);
            for (unsigned int i = 0; i< lucidmem->evictPoss.size(); i++){
                auto writeevict =  lucidmem->evictPoss[i];
                // evict data, get the tail node, remove the tail of intrachain
                gettimeofday(&lt1, NULL);

                auto evictleaf = lucidmem->lru->get_tail1();

                if (evictleaf->value->buffer_piece == NULL){
                    lucidadatype::sortPiece * evictbuffer = (lucidadatype::sortPiece*) (evictleaf->value);

                    lucidmem->lru->remove(evictleaf->key);
                    ...
                    for (int i = 0; i < evictbuffer->slotkey.size();i ++ ){
                        ...
                        auto writeevict = lucidmem->reduced_evictkeytoinpage(evictbuffer->slotkey[i], (key_type*)evictbuffer->slotdata[i],
                                                                            diskpage);

                        evictbuffer->slotflag[i] = false;
                    ...
                    gettimeofday(&lt2, NULL);
                    ltimeuse += (lt2.tv_sec - lt1.tv_sec) + (double) (lt2.tv_usec - lt1.tv_usec) / 1000000.0;
                    auto writeevict = lucidmem->reduced_evictkeytoinpage(evictleaf->value->slotkey[evictslotV->key], evictslotV->value,
                                                                        diskpage);
            ...
            lucidindex->inkeynum -= lucidmem->evictPoss.size();
            lucidindex->exkeynum += lucidmem->evictPoss.size();
            lucidmem->evictPoss.clear();
            // 批量将inmempages写出磁盘
            int pagenum = lucidmem->runtimeevictpagestodisk(diskpage);
            ...
            double ratio = (totalusemem-lucidmem->threshold)/lucidmem->threshold  ;
            unsigned int batch_evict = ceil(lucidindex->inkeynum*ratio*0.6) + 300 ;
            if (ratio>0.2){
                batch_evict = ceil(lucidindex->inkeynum*ratio*0.5);
            }
            gettimeofday(&wdt1, NULL);
            for (unsigned int i = 0; i< batch_evict; i++)   // the number of records to be evicted in batches
            {
                ...
                if (evictleaf->value->buffer_piece == NULL){
                    ...
                    auto writeevict = lucidmem->reduced_evictkeytoinpage(evictleaf->value->slotkey[evictslotV->key], evictslotV->value,
                                                                        diskpage);
//                    auto writeevict = lucidmem->evictkeytoinpage(evictleaf->value->slotkey[evictslotV->key], evictslotV->value,
//                                                                diskpage);
            ...
            lucidindex->inkeynum -= batch_evict;
            lucidindex->exkeynum += batch_evict;
    ...
    double runtimeevictkeytopage2(lucidadamemory *lucidmem,double totalusemem, lucidadatype *lucidindex,lucidadadisk *diskpage , access_stats *r_stats){

        double wdtimeuse =0.0;  // 写磁盘的时间
//        double ltimeuse = 0.0;

        if (lucidmem->reused_index_pageid.size() != 0){
//            gettimeofday(&wdt1, NULL);

            auto reused_pagenum = lucidmem->reused_index_pageid.size();   // 判断是否有 reused page
            // 根据 内存占用，判断要写出的数据量，以及要使用的reused_page
            double ratio = (totalusemem-lucidmem->threshold)/lucidmem->threshold;
            unsigned int batch_page_num = std::min<int>(ceil(lucidindex->inkeynum*ratio*0.7)/diskpage->recordnum + 50,reused_pagenum);
            auto wstart_time = std::chrono::high_resolution_clock::now();
            for (unsigned int i = 0; i< batch_page_num; i++){
                // 读取reused page，并且一次写满该page
                unsigned int reused_pageid = lucidmem->reused_pageids[i];
                unsigned long reused_index_pageid = lucidmem->reused_index_pageid[i];
                // 为该page 创建一个inpage in memory
                auto reused_page = lucidmem->createinmempage(reused_pageid,diskpage->pagesize,diskpage->recordnum);
...
//                                .count();
                        // 什么时候remove    lucidmem->lru->remove(evictleaf->key);
                        /*
                        for (int j = 0; j < evictbuffer->slotkey.size();j ++ ){
                           if (evictbuffer->slotflag[j]){
                               auto writeevict = lucidmem->reduced_evictkeytoinpage(evictbuffer->slotkey[j], (key_type*)evictbuffer->slotdata[j],
                                                                                   diskpage,reused_page, reused_index_pageid);
                               evictbuffer->slotflag[j] = false;
                               ...
                               {
                                   lucidmem->inmempages.emplace_back(reused_page);
                                   if (++i == reused_pagenum)
                                       break;
                                   reused_pageid = lucidmem->reused_pageids[i];
                                   reused_index_pageid = lucidmem->reused_index_pageid[i];
                                   // 为该page 创建一个inpage in memory
                                   reused_page = lucidmem->createinmempage(reused_pageid,diskpage->pagesize,diskpage->recordnum);
                               }
                           }
                        }
                        */
                        ...
                        for (int k = 0; k < evictbuffer->data_capacity_;k++){
                            if (evictbuffer->check_exists(k)){
                                ...
                                auto writeevict = lucidmem->reduced_evictkeytoinpage(evictbuffer->ALEX_DATA_NODE_KEY_AT(k),
...
//                                std::chrono::high_resolution_clock::now() - lstart_time)
//                                .count();   // ns
                        auto writeevict = lucidmem->reduced_evictkeytoinpage(evictleaf->value->slotkey[evictslotV->key], (key_type*)evictslotV->value,
                                                                            diskpage,reused_page, reused_index_pageid);
...
//                delete reused_page;
            ...
            if (lucidmem->inmempages.size()>batch_page_num)
                lucidmem->inmempages.pop_back();

            lucidindex->inkeynum -= (diskpage->recordnum * batch_page_num);
            lucidindex->exkeynum += (diskpage->recordnum * batch_page_num);

            if (batch_page_num == reused_pagenum){ //不能再释放所有的reused_pageids,需要判断
                vector<unsigned long >().swap(lucidmem->reused_index_pageid);
                vector<pageid_type>().swap(lucidmem->reused_pageids);
            }
            else{
                lucidmem->reused_index_pageid.assign(lucidmem->reused_index_pageid.begin()+batch_page_num,lucidmem->reused_index_pageid.end());
                lucidmem->reused_pageids.assign(lucidmem->reused_pageids.begin()+batch_page_num,lucidmem->reused_pageids.end());
            }

            // 需要单独将每个page 写出去磁盘，因为reused 的page 不一定连续
            int pagenum = lucidmem->runtimeevictreusedpagestodisk(diskpage);
            r_stats->writenum += pagenum;
...
//            typename std::unordered_map<int,vector<v_type>>::const_iterator got = prepass.find(pageid);
    ...
    typedef prepass_dict< pair< lucidadatype::Leafpiece* ,pair< lruOff_type, pageOff_type > > > pre_dict ;

// in the range query, judge whether the keys in each overlapped leaf in memory or in disk
    void range_is_in_or_out(int start, int end, lucidadatype::Leafpiece* curleaf ,vector<pair<key_type,key_type*>> *totalres , pre_dict* dict,int sizevalue ,lucidadamemory *lucidmem,access_stats *r_stats){
        ...
        for (int k = start;k < end;k++)
        {
            ...
            if (flag){
                ...
                lucidmem->lru->put(curleaf->startkey,curleaf);                 // 更新global lru
                gettimeofday(&lt2, NULL);
                ltimeuse = (lt2.tv_sec - lt1.tv_sec) + (double) (lt2.tv_usec - lt1.tv_usec) / 1000000.0;
                r_stats->lrutime += ltimeuse;
            }
                ...
                dict->insert_dict(pageid, pair< lucidadatype::Leafpiece*,pair<lruOff_type,pageOff_type > >(curleaf,pair<lruOff_type,pageOff_type>(k,off)));
...
//                cout<<"my Lord, my heart is designed for You!"<<endl;
    ...
    void htap_range_is_in_or_out(int start, int end, lucidadatype::Leafpiece* curleaf ,vector<pair<key_type,key_type*>> *totalres , pre_dict* dict,
                                 int sizevalue ,lucidadamemory *lucidmem,access_stats *r_stats,int reduce_factor){
        struct timeval lt1, lt2;
        double ltimeuse;
        for (int k = start;k < end;k++)
        {
            ...
            if (flag){
                ...
                auto int_high_low = lucidmem->reduced_evicttable[int_reduced_evict];
...
//                                unsigned short lo = hhh&0xFFFF ;// 低16位
                ...
                reduced_writeevict.first = lucidmem->reduced_evicttable[index_pageid];
                // 将 querykey merge到memory，修改 bitmap，修改num in page
//                            auto numinpage = memorylucid.reduced_evicttable[index_pageid+1];
//                lucidmem->reduced_evicttable[index_pageid+1] -= 1;
...
//                cout<<"my Lord, my heart is designed for You!"<<endl;
    ...
    void htap_range_is_in_or_out(int start, int end, lucidadatype::sortPiece* curleaf ,vector<pair<key_type,key_type*>> *totalres , pre_dict* dict,int sizevalue ,lucidadamemory *lucidmem,access_stats *r_stats,int reduce_factor){
        struct timeval lt1, lt2;
        double ltimeuse;
        for (int k = start;k < end;k++)
        {
            if (curleaf->check_exists(k))  // 如果不是gap
            {
                ...
                if (flag){
                    ...
                    lucidmem->lru->put(curleaf->ALEX_DATA_NODE_KEY_AT(0),curleaf);                 // 更新global lru
                    gettimeofday(&lt2, NULL);
                    ltimeuse = (lt2.tv_sec - lt1.tv_sec) + (double) (lt2.tv_usec - lt1.tv_usec) / 1000000.0;
                    r_stats->lrutime += ltimeuse;
                }
...
//                cout<<"my Lord, my heart is designed for You!"<<endl;
    ...
    int range_prepass(lucidadamemory *lucidmem,lucidadatype::range_res_find *index_result,vector<pair<key_type,key_type*>> *result,pre_dict* dict,lucidadatype *lucidindex,access_stats *r_stats){  // 将range query 由btree 得到的信息，进行怕热pass，遍历find leaf中的每一个leaf，如果slotdata.first = true, 则直接放入reslut 中，否则将其放入prepassdict 中

        int leafnum = index_result->findleafs.size();
        if (leafnum == 1){  //all the keys in range belonging to a certain leaf
            range_is_in_or_out(index_result->firstslot,index_result->lastslot+1,index_result->findleafs[0],result,dict,lucidindex->valuesize,lucidmem,r_stats);
        }
        else {  // the first leaf and last leaf need to be deal with seperately
            // deal with firstleaf
            //index_result->findleafs[0];
            range_is_in_or_out(index_result->firstslot,index_result->findleafs[0]->slotkey.size(),index_result->findleafs[0],result,dict,lucidindex->valuesize,lucidmem,r_stats);
            // deal with the middle leafs
            for (int i=1; i < leafnum-1; i++){
                range_is_in_or_out(0,index_result->findleafs[i]->slotkey.size(),index_result->findleafs[i],result,dict,lucidindex->valuesize,lucidmem,r_stats);
            }
            //deal with lastleaf
            //index_result->findleafs[leafnum-1];
            range_is_in_or_out(0,index_result->lastslot+1,index_result->findleafs[leafnum-1],result,dict,lucidindex->valuesize,lucidmem,r_stats);
        }
        return 0;
    }

    int htap_range_prepass(lucidadamemory *lucidmem,lucidadatype::htaprange_res_find *index_result,vector<pair<key_type,key_type*>> *result,pre_dict* dict,lucidadatype *lucidindex,access_stats *r_stats, int reduce_factor){  // 将range query 由btree 得到的信息，进行怕热pass，遍历find leaf中的每一个leaf，如果slotdata.first = true, 则直接放入reslut 中，否则将其放入prepassdict 中

        int leafnum = index_result->findleafs.size();
        if (leafnum == 1){  //all the keys in range belonging to a certain leaf
//            cout << "Lord, this is chao" << endl;
            htap_range_is_in_or_out(index_result->slotleaf0,index_result->slotleaf1+1,index_result->findleafs[0],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
        }
        else {  // the first leaf and last leaf need to be deal with seperately
            // deal with firstleaf
//            cout << "Lord, this is chao" << endl;
            htap_range_is_in_or_out(index_result->slotleaf0,index_result->findleafs[0]->slotkey.size(),index_result->findleafs[0],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
            // deal with the middle leafs
            for (int i=1; i < leafnum-1; i++){
                htap_range_is_in_or_out(0,index_result->findleafs[i]->slotkey.size(),index_result->findleafs[i],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
            }
            //deal with lastleaf
//            cout << "Lord, this is chao" << endl;
            htap_range_is_in_or_out(0,index_result->slotleaf1+1,index_result->findleafs[leafnum-1],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
        }
...
//                if (index_result->findbuffers[0]->num_keys_ > 0)
                if (index_result->slotbuffer0!=index_result->slotbuffer1) {
                    ...
                    auto buffer = (lucidadatype::sortPiece *) index_result->findbuffers[0];
...
//                                            buffer, result, dict,
//                                            lucidindex->valuesize, lucidmem, r_stats, reduce_factor);
                ...
                // deal with firstleaf
//                cout << "Lord, this is chao" << endl;
                lucidadatype::sortPiece* buffer = (lucidadatype::sortPiece*) (index_result->findbuffers[0]);
...
//                htap_range_is_in_or_out(index_result->slotbuffer0,buffer->data_capacity_,buffer,result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
                htap_range_is_in_or_out(start_buffer0,end_buffer0,(lucidadatype::sortPiece*)index_result->findbuffers[0],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
                // deal with the middle leafs
                for (int i=1; i < buffernum-1; i++){
                    buffer = (lucidadatype::sortPiece*) (index_result->findbuffers[i]);
                    // 找到该buffer 的第一个valid record 的位置
                    auto start_buffer = buffer->find_first_1();  //需要考虑
                    auto end_buffer = buffer->find_last_1()+1;  // 需要考虑
//                    htap_range_is_in_or_out(0,buffer->data_capacity_,index_result->findbuffers[i],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
                    htap_range_is_in_or_out(start_buffer,end_buffer,index_result->findbuffers[i],result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
//                    COUT_THIS("thank You, my lovely Lord");
                }
                //deal with lastleaf
//                cout << "Lord, this is chao" << endl;
                buffer = (lucidadatype::sortPiece*)(index_result->findbuffers[buffernum-1]);
                auto start_buffer = buffer->find_first_1();  // 需要考虑。找到该buffer 的第一个valid record 的位置
//                auto end_buffer = buffer->find_last_1();  // 不需要考虑
//                htap_range_is_in_or_out(0,index_result->slotbuffer1,buffer,result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
                htap_range_is_in_or_out(start_buffer,index_result->slotbuffer1,buffer,result,dict,lucidindex->valuesize,lucidmem,r_stats,reduce_factor);
            }
        }
    ...
    // 一次至少读取1024k 的数据
    int range_read_from_disk(vector<pair<key_type,key_type*>> *result,pre_dict* dict,lucidadamemory *lucidmem,
                             lucidadatype *lucidindex,lucidadadisk *pagedisk, access_stats *r_stats, key_type* payload) {
        // 打开磁盘文件
...
//                auto iiii = lucidmem->reduced_evicttable[index_pageid];
//                if (pageid == 3027)
...
//                    auto writeevict = v.second[k].second.second;
                    ...
                    if (v.first == lucidmem->inpage->pageid) {
                        //vector<key_type> inmemdata = lucidmem->inpage->inmemdata;
                        key_type *inmemdata = lucidmem->inpage->inmemdata;
                        res.first = inmemdata[off];
                        res.second = new key_type[lucidindex->valuesize];
                        for (int i = 0; i < lucidindex->valuesize; i++) {
                    ...
                    lucidmem->unset_bit(recordnum, index_pageid);
                    lucidmem->reduced_evicttable[index_pageid+1] -= 1;
                    lucidindex->inkeynum++;
                    lucidindex->exkeynum--;
                    ...
                    if (v.second[k].first->buffer_piece != NULL){
                        lucidadatype::Leafpiece *curpiece = v.second[k].first;
...
//                        curpiece->slotdata[slot] = curpiece->intrachain.put(slot, res.second);
                        ...
                        lucidmem->lru->put(curpiece->startkey, curpiece);// 更新 interchain
                        gettimeofday(&lt2, NULL);
                        ltimeuse += (lt2.tv_sec - lt1.tv_sec) + (double) (lt2.tv_usec - lt1.tv_usec) / 1000000.0;
                    }
                    else{
                        lucidadatype::sortPiece *curpiece = (lucidadatype::sortPiece *)v.second[k].first;
...
//                        curpiece->ALEX_DATA_NODE_PAYLOAD_AT(slot) = res.second;
                        ...
                        // 将curpiece 插入到interchain 中，即mem->lru中
                        lucidmem->lru->put(curpiece->ALEX_DATA_NODE_KEY_AT(0), curpiece);// 更新 interchain
                        gettimeofday(&lt2, NULL);
                        ltimeuse += (lt2.tv_sec - lt1.tv_sec) + (double) (lt2.tv_usec - lt1.tv_usec) / 1000000.0;
                    }
                ...
                if (lucidmem->check_merge(index_pageid) == true){
                    if (pageid != lucidmem->inpage->pageid){
//                        auto numinpage = lucidmem->reduced_evicttable[index_pageid+1];
                        lucidmem->reused_pageids.emplace_back(pageid);
                        lucidmem->num_page_merge += 1;
                        lucidmem->reused_index_pageid.emplace_back(index_pageid);
                        auto pagenum = lucidmem->reduced_evicttable[index_pageid+1];
                        lucidmem->page_merge(buf,pagedisk->recordnum, index_pageid,offset);
                        pagenum = lucidmem->reduced_evicttable[index_pageid+1];
                        lucidindex->exkeynum -= lucidmem->reduced_evicttable[index_pageid+1];
                        lucidindex->inkeynum += lucidmem->reduced_evicttable[index_pageid+1];
                        lucidmem->reduced_evicttable[index_pageid+1] = 0;
                    }
//                    else{
...
//                        lucidmem->num_page_merge += 1;
//                        lucidmem->reused_index_pageid.emplace_back(index_pageid);
//                        lucidmem->page_merge(lucidmem->inpage->inmemdata,pagedisk->recordnum, index_pageid);
//                        lucidmem->inpage->pageid +=1;
//                        lucidmem->inpageid ++;
//                        lucidmem->inpage->recordnum = 0;
//                        lucidmem->inpage->freespace = pagedisk->pagesize;
//                        lucidmem->inpage->usedspace = 0;
//
//                        lucidindex->exkeynum -= lucidmem->reduced_evicttable[index_pageid+1];
//                        lucidindex->inkeynum += lucidmem->reduced_evicttable[index_pageid+1];
//                        lucidmem->reduced_evicttable[index_pageid+1] = 0;
...
//                        auto writeevict = v.second[vi].second.second;
                        ...
                        if (v.second[vi].first->buffer_piece != NULL){
                            ...
                            lucidadatype::Leafpiece *curpiece = v.second[vi].first;
                            res.first = inmemdata[off];
                            res.second = new key_type[lucidindex->valuesize];
                            for (int ki = 0; ki < lucidindex->valuesize; ki++) {
                                res.second[ki] = inmemdata[off + 1 + ki];
                            }// check page merge
                            delete [] res.second;
                            auto recordnum = off/pagedisk->recordsize;  // compute the key's rank in the page
//                        bool former_flag = lucidmem->check_exists(recordnum,index_pageid);
//                        if (former_flag == false){
////                auto numinpage = memorylucid->pageinfo[index_pageid+1];
...
//                      cout<< " i need You, my Lord!!! we all need You! "<< endl;
                            ...
                            lucidadatype::sortPiece *curpiece = (lucidadatype::sortPiece *)v.second[vi].first;
...
//                cout<< " i need You, my Lord!!! we all need You! "<< endl;
                ...
                if (lucidmem->check_merge(index_pageid) == true){
                    ...
                    if (pageid != lucidmem->inpage->pageid) {
                        ...
                        lucidmem->num_page_merge += 1;
                        lucidmem->reused_index_pageid.emplace_back(index_pageid);
                        lucidmem->page_merge(buf, pagedisk->recordnum, index_pageid);
...
//                    }
    ...
    int test_interleave_insert_query(unsigned int errbnd,size_t numkey,int pagesize,string dataset,double mem_threshold,double reserveMem,
        ...
        lucidadatype* lucidada = new lucidadatype(0, errbnd, errbnd);
        lucidadalrutype *interchain = new lucidadalrutype(actual_numkey);
//        auto kkk = int (actual_numkey/600);
        lucidada->leaflevel.leafpieces.reserve(int (actual_numkey/600));
        lucidada->valuesize = recordSize - 1;
        lucidada->buffer_ratio = work.buffer_ratio/(1-work.buffer_ratio);
        ...
        ossr << (recordSize);
        ossd << actual_numkey;
        string diskpath = "/home/wamdm/chaohong/clionDir/updatelucid/diskpath/";
        string file_str =
                diskpath + dataset + "_totaly_rebuild_lucidadapages_flattened_" + ossd.str() + "_" + ossm.str() + "_" + ossp.str() + "_" +
                ossr.str() + "_" + osse.str();
        const char *diskfile = file_str.c_str();
        int numrecord = pagesize / (recordSize);
        int bitnum = std::max(numrecord/32,1);
        lucidstorage::lucidmemory<key_type, key_type *, lucidadatype, lucidadalrutype, lucidadadisk> memorylucid(numkey,
                                                                                                           mem_threshold,
                                                                                                           lucidada,
                                                                                                           interchain,bitnum);
        memorylucid.reserveMem = reserveMem;
        memorylucid.merge_threshold = merge_threshold;
        lucidstorage::luciddisk<key_type> disklucid(diskfile, pagesize, numrecord, recordSize, merge_threshold);
        if (numrecord <32)
            disklucid.reduce_factor = 11;
        fstream fs;
        ...
        key_type payload[lucidada->valuesize]{};
        key_type update_payload[lucidada->valuesize]{1};
        ...
        memorylucid.append(work.init_keys, payload, errbnd, errbnd);    // 执行数据bulk load ，逐个更新
        ...
        pair<bool, memoryusage> transflag = memorylucid.judgetransfer();
        unsigned int transleaves;
        double ratio;
        unsigned int leaves = transflag.second.meminfo["leaves"];
        if (memorylucid.inpage == NULL) {
            memorylucid.createinmempage(disklucid.pagesize, disklucid.recordnum);    //create a page in memory;
            memorylucid.newpage_reduced_evicttable(0);  // memorylucid.inpage->pageid = 0
        }

        // evitct according to lru
        while (transflag.first) {
            initwritetime += memorylucid.lucidtransfer(transflag.second.meminfo["totalusage"], &disklucid);
            transflag = memorylucid.judgetransfer();
        }

        ofstream savefile;
        string performance_file = "/home/wamdm/chaohong/clionDir/updatelucid/result/lucid_performance.txt";
        savefile.open(performance_file, ios::app);
        savefile << "method " << "lucid+flattened " << "available_memory " << mem_threshold + reserveMem << " error " << errbnd << " pagesize " << (pagesize * 8 / 1024) ;
        ...
        savefile << "init_state " << "lucid+flattened ";
        savefile << "init_write_time " << disklucid.initwtime / 1000000.0  << " ";
        savefile << "pages_init_num " << disklucid.nextpageid << " ";
...
//        double querycumulative_writetime = 0.0;
        ...
        transflag = memorylucid.judgetransfer(&query_stats);
        int periodV = memorylucid.reserveMem * 1024 * 1024 / ((lucidada->valuesize + 1) * sizeof(key_type) * 5);
        enum Operation {
        ...
        while (true) {
            ...
            if (i > 0) {
                ...
                double batch_query_writetime = 0.0, bactch_lru_write_time =0;
...
//                        if ( batch_no == 10 && qi == 88916){
//                            cout << "Jesus, where this key go?" << endl;
//                        }
                        pair<key_type, key_type *> res = memorylucid.get_key(op_key, &query_stats, &disklucid);
                        if (query_stats.disknum > 0 && qi++ % periodV == 0) {
//                            gettimeofday(&insert_start_time, NULL);
                            transflag = memorylucid.judgetransfer(&query_stats);
                            while (transflag.first) {
                                batch_query_writetime += runtimeevictkeytopage2(&memorylucid, transflag.second.meminfo["totalusage"],
                                                                                lucidada, &disklucid, &query_stats);
                                transflag = memorylucid.judgetransfer(&query_stats);
                            }
...
//                        cout << "this is read, thank You, my Lord" << endl;
                ...
                unsigned last_retrain = lucidada->retrain;
                int last_newmodels = lucidada->newmodels;
...
//                   lucidadatypeinsert_random(work.insert_ops, payload, interchain);
...
//                        }
//                       lucidada->insert_one(op_key,payload,interchain);
                        lucidada->insert(op_key,payload,interchain);
                    }
//                    cout << "i need You, my Lord" << endl;
                    // used in insert_one procedure, update the last piece
                    /*
                   lucidada->root =lucidada->innerlevels.back()->innerpieces[0];
                   lucidada->inkeynum += num_actual_inserts;
                    auto a =lucidada->leaflevel.opt->get_segment(lucidada->m_tailleaf->endkey);
                    if (lucidada->leaflevel.opt->points_in_hull< 2){
                        a.first =lucidada->m_tailleaf->endkey;
                        a.last =lucidada->m_tailleaf->endkey;
                    }
                   lucidada->m_tailleaf->update(a);
                    */
                    insert_elapsed_time =
                    ...
                    query_stats.computetimeuse = 0.0;
                    while (transflag.first) {
                        batch_insert_writetime += runtimeevictkeytopage2(&memorylucid, transflag.second.meminfo["totalusage"],
                                                                         lucidada, &disklucid, &query_stats);
                        transflag = memorylucid.judgetransfer(&query_stats);
                    }
                    ...
                    batch_newmodels =lucidada->newmodels - last_newmodels;
                    batch_retrain =lucidada->retrain - last_retrain;
                    total_retrain += batch_retrain;
                    total_newmodels += batch_newmodels;

                    if (lucidada->retrain >= rebuild_threshold){

                        auto start_time = std::chrono::high_resolution_clock::now();
                        // rebuild internal levels
                        lucidada->internal_level_rebuild(errbnd,interchain);

                        rebuild_elapsed_time = std::chrono::duration_cast<std::chrono::nanoseconds>(
                                std::chrono::high_resolution_clock::now() - start_time)
                                .count();
//                        lucidada->newmodels = 0;
                        lucidada->retrain = 0;
                        lucidada->rebuild_inner += 1;
//                        gettimeofday(&lookup_end_time, NULL);
...
//                        }
                        // 先找到key，再更新
                        pair<key_type, key_type *> res = memorylucid.update_key(op_key, update_payload, &query_stats,disklucid);
...
////                        pair<key_type, key_type *> res = memorylucid.get_scan(op_key, &query_stats,disklucid);
...
//                        }
                        ...
                        auto index_res = memorylucid.get_range(firstkey,lastkey, &query_stats,disklucid);

//                        auto index_res = memorylucid.get_scan(firstkey,scan_num, &query_stats,disklucid);
                        pre_dict *dict = new pre_dict();
                        htap_range_prepass(&memorylucid, &index_res, &rres, dict, lucidada, &query_stats,disklucid.reduce_factor);
                        ...
                        if (dict->pagenum == 0) {  // the request data  数据都在内存
                            ...
                            range_read_from_disk(&rres, dict, &memorylucid, lucidada, &disklucid,
                            ...
                            range_read_from_disk(&rres, dict, &memorylucid, lucidada, &disklucid, &query_stats,payload);
                        }
                        delete dict;
...
//                workload_elapsed_time -= fill_elapsed_time;
                ...
                if (true) {
                    ...
                    map<string, double>::iterator iter;
//                savefile << "_ ";
                    savefile << "disk_write_time " << disklucid.initwtime / 1000000.0  << " ";
                    savefile << "used_page_num " << (disklucid.nextpageid-memorylucid.reused_pageids.size()) << " ";
                    savefile << "method " << "lucid+flattened ";
                    for (iter = transflag.second.meminfo.begin(); iter != transflag.second.meminfo.end(); iter++)
                        savefile << iter->first << " " << iter->second << " ";
//                savefile << "\n";
                    savefile << flush;
                    ...
                    std::cout << "\n\tcumulative_execution_1:\t"
                              << ((lucidada->inkeynum +lucidada->exkeynum)/double(1024 * 1024)) * (lucidada->valuesize + 1) * 8
                              << " G datasize, \t"
                              <<lucidada->inkeynum +lucidada->exkeynum << " #cumulative_keys,\t"
                              <<lucidada->inkeynum << " #inmem_keys,\t"
                              <<lucidada->exkeynum << " #exmem_keys"
                              << "\n\tcumulative_execution_1:\t"
                            ...
                            << " cumulative_range_time,\t"
                             << total_retrain << " cumulative_retrain,\t "
                              ...
                              << (disklucid.initwtime / 1000000.0) + query_stats.wdisktime << " cumulative_writetime "
                              << cumulative_insert_writetime << " cumulative_insert_writetime "
                              << cumulative_update_time << " cumulative_update_time ";

                    std::cout << "\n\tlucid_update_1:\t"
                              << rebuild_elapsed_time << " rebuild_elapsed_time "
                              << memorylucid.num_page_merge << " num_page_merge "
                              << memorylucid.merge_timeuse << " merge_timeuse "
                            ...
                            <<lucidada->reserve_buff << " reserve_buff "
                            <<lucidada->key_in_buff << " key_in_buff "
                            <<lucidada->fill_factor << " fill_factor ";
                    ...
                    savefile << batch_no << " Batch " <<
                            ...
                            << cumulative_range_keys / cumulative_time << " cum_keys/sec "
                             ...
                             << cumulative_insert_writetime << " cumulative_insert_writetime "
                             << rebuild_elapsed_time << " rebuild_elapsed_time "
                             <<lucidada->rebuild_inner << " num_of_inner_rebuild "
                            << total_retrain << " cumulative_retrain "
                            << total_newmodels << " cumulative_new_models "
                            << total_retrain << " cumulative_retrain "
                            <<  comulative_rebuild_time << " comulative_rebuild_time"
                            << batch_retrain<< " batch_times_of_leaves_retrain "
                            << batch_newmodels << " batch_updated_new_models "
                            <<lucidada->maxbsize  << " maxbuffersize "
                            <<lucidada->minbsize  << " minbuffersize "
                            <<lucidada->reserve_buff << " reserve_buff "
                            <<lucidada->key_in_buff << " key_in_buff "
                            <<lucidada->fill_factor << " buff_fill_factor ";

                    auto allkeynum =lucidada->inkeynum +lucidada->exkeynum;
                    savefile << (lucidada->inkeynum +lucidada->exkeynum) * (lucidada->valuesize + 1) * 8 / double(1024 * 1024)
                             << "G datasize "
                             << allkeynum << " #cumulative_keys "
                             << allkeynum/double(allkeynum+lucidada->reserve_buff) << " glabal_fill_factor "
                             <<lucidada->inkeynum << " #inmem_keys "
                             <<lucidada->exkeynum << " #exmem_keys "
                    ...
                    savefile << flush;
                    savefile.close();
                    lucidada->key_in_buff = 0;
                    lucidada->reserve_buff = 0;
                    lucidada->fill_factor = 0.0;

                }


                // Check for workload end conditions
                if (workload_elapsed_time > time_limit || (lucidada->inkeynum +lucidada->exkeynum) >= actual_numkey) {
                    break;
                }
...
//                }
        ...
        savefile << flush;
        savefile.close();
        vector<unsigned int>().swap(memorylucid.reduced_evicttable);
        fs.open(diskfile,ios::in);
        delete lucidada;
        if (fs){
            fs.close();
